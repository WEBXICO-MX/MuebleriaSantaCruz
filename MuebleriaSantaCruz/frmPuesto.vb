...
Public Class frmPuesto
    ...
    Public Sub Nuevo()
        ...
        'Crear una consulta
        Dim Consulta As String = "SELECT id FROM Puestos ORDER BY id DESC"
    ...
    Public Sub InsertaNuevo()
        ...
        'Crear una consulta
        Dim Consulta As String = "INSERT INTO Puestos (id, nombre,activo) VALUES (" & txtID.Text & ",'" & txtNombre.Text & "'," & (If(cbxActivo.Checked, 1, 0)) & ")"
        Orden = New SqlCommand(Consulta, objcon.con)
        ...
        Try
            ...
            Me.PuestosTableAdapter.Fill(Me.DataSetPuesto.puestos)

            EstadoBotones(True, False, False, False, True)
            EstadoCajasdeTexto(False)

            For Fila = 0 To PuestosDataGridView.Rows.Count - 1
                If (PuestosDataGridView.Item(1, Fila).Value = txtNombre.Text) Then
                    PuestosDataGridView.Item(0, Fila).Selected = True
                    Exit For
                End If
            Next Fila
            ...
            MessageBox.Show("El puesto '" & txtNombre.Text & "' ha sido guardado", "Guardar", MessageBoxButtons.OK)

            LimpiarCajasdeTexto()
            lbtipoestado.Visible = False
            PuestosDataGridView.Enabled = True
        Catch ex As SqlException
    ...
    Public Sub Editar()
        ...
        'Crear una consulta
        Dim Consulta As String = "UPDATE Puestos SET nombre = '" & txtNombre.Text & "', activo = " & (If(cbxActivo.Checked, 1, 0)) & " WHERE id = " & txtID.Text
        Orden = New SqlCommand(Consulta, objcon.con)
        ...
        Catch ex As SqlException
            If (ex.Number = 2601) Then
                MessageBox.Show("El puestoe ya existe", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
            ElseIf (ex.Number = 2627) Then
                MessageBox.Show("El ID ya existe", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Else
                MsgBox("Error: " + ex.Message)
    ...
    Private Sub PegarDatosTabla_CajasdeTexto(ByVal F As Integer)
        txtID.Text = PuestosDataGridView.Rows(F).Cells(0).Value
        txtNombre.Text = PuestosDataGridView.Rows(F).Cells(1).Value
        cbxActivo.Checked = PuestosDataGridView.Rows(F).Cells(2).Value
    End Sub
    ...
    Private Sub PuestosDataGridView_CellClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles PuestosDataGridView.CellClick
        If (PuestosDataGridView.Enabled = True) Then
    ...
    Private Sub PuestosDataGridView_RowEnter(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles PuestosDataGridView.RowEnter
        If (PuestosDataGridView.Enabled = True) Then
            If (e.RowIndex >= 0) Then
                PegarDatosTabla_CajasdeTexto(e.RowIndex)
                EstadoBotones(True, False, True, True, True)
    ...
    Private Sub btnuevo_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnuevo.Click
        ...
        PuestosDataGridView.Enabled = False
    ...
    Private Sub btguardar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btguardar.Click
        ...
        If (txtNombre.Text.Length = 0) Then
            ErrorProvider1.SetError(txtNombre, "Capture el nombre del puesto")
            txtNombre.Focus()
            Exit Sub
        End If
    ...
    Private Sub bteditar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles bteditar.Click
        If (txtID.Text <> "") Then
            Dim F As Integer = PuestosDataGridView.CurrentCellAddress.Y
            ...
            PuestosDataGridView.Enabled = False
        Else
            MessageBox.Show("Seleccione una puesto para editar", "Editar", MessageBoxButtons.OK, MessageBoxIcon.Information)
        End If
    End Sub
    ...
    Private Sub frmPuesto_Shown(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Shown
        ...
        If (PuestosBindingSource.Count = 0) Then
            EstadoBotones(True, False, False, False, False)
        Else
            EstadoBotones(True, False, False, False, True)
        End If

        PuestosDataGridView.ClearSelection()
    End Sub

    Private Sub frmPuesto_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        'TODO: esta línea de código carga datos en la tabla 'DataSetPuesto.puestos' Puede moverla o quitarla según sea necesario.
        Me.PuestosTableAdapter.Fill(Me.DataSetPuesto.puestos)
        Me.Top = 85
